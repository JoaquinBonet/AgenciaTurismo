...
public class ItinerarioDAOImpl implements ItinerarioDAO {
	...
	public ArrayList<Producto> findPromosItinerarioByUser(Usuario user) {
		...
		try {
			...
			String sql = "SELECT PROMOCIONES.id_promo, PROMOCIONES.id_tipo_producto, PROMOCIONES.tipo_promo, PROMOCIONES.nombre, PROMOCIONES.id_tipo_atraccion, PROMOCIONES.descuento, PROMOCIONES.precio, PROMOCIONES.info, PROMOCIONES.img_url FROM PROMOCIONES JOIN ITINERARIO  WHERE ITINERARIO.id_tipo_producto = PROMOCIONES.id_tipo_producto AND ITINERARIO.id_item = PROMOCIONES.id_promo AND ITINERARIO.id_usuario = ?";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
			statement.setInt(1, id);
			ResultSet resultados = statement.executeQuery();
